...
export default function MoreTrainInformation() {
    ...
    const infoCards = [
        { label: "IRCTC Train Booking", icon: "fas fa-train", href: "/" },
        { label: "PNR Status Enquiry", icon: "fas fa-ticket-alt", href: "/pnr-status-enquiry" },
        { label: "Train Running Status", icon: "fas fa-map-marker-alt", href: "/running-status" },
        ...
        { label: "Train Stations", icon: "fas fa-train-subway", href: "/search-station" },
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                {infoCards.map((card, idx) => (
                    <Link
                        key={idx}
                        href={card.href}
                        className="flex items-center justify-between bg-blue-100 hover:bg-blue-200 transition px-4 py-6 rounded shadow-sm"
                    >
                        <div className="flex items-center gap-3">
                            <i className={`${card.icon} text-blue-700 text-lg`}></i>
